...
export class CoreToolScheduler {
  ...
  private async _processNextInQueue(signal: AbortSignal): Promise<void> {
    ...
    if (toolCall.status === 'error') {
      this.completedToolCallsForBatch.push(toolCall as CompletedToolCall);
  ...
  private async checkAndNotifyCompletion(signal: AbortSignal): Promise<void> {
    // This method is now concerned with the single active tool call.
    if (this.toolCalls.length === 0) {
      ...
      // Clear the active tool slot. This is crucial.
      this.toolCalls = [];
      this.notifyToolCallsUpdate();
    }

    // If the queue is empty or the operation was cancelled, we finalize the batch.
    if (this.toolCallQueue.length === 0 || signal.aborted) {
      if (signal.aborted) {
        this._cancelAllQueuedCalls();
      }

      // Guard against firing the completion handler multiple times for the same batch.
      if (this.completedToolCallsForBatch.length === 0) {
        return;
      }
      ...
      // After completion of the entire batch, process the next item in the request queue.
      ...
      // Otherwise, continue processing the current batch sequence.
      await this._processNextInQueue(signal);
    }
  }
