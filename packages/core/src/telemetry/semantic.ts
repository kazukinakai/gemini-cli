...
export function toOTelPart(part: Part): AnyPart {
  if (part.thought) {
    // thought boolean indicates this is a ReasoningPart, no need to include the field
    const { thought: _thought, ...data } = part;
    if (data.text) {
      const { text, ...unexpectedData } = data;
      return new ReasoningPart(text, { ...unexpectedData });
    } else {
      return new ReasoningPart('', { ...data });
    }
  } else if (part.text) {
    const { text, ...unexpectedData } = part;
    return new TextPart(text, { ...unexpectedData });
  } else if (part.functionCall) {
    const { functionCall, ...unexpectedData } = part;
    ...
    const { functionResponse, ...unexpectedData } = part;
...
export class TextPart {
  ...
  constructor(content: string, otherData?: { [key: string]: unknown }) {
